...
function getTop5Search(){
	...
	$.indexedDB("jqm-fav").objectStore("log").each(function(row){
				...
				if ( row.value.RetlName )
					retlNames.unshift( row.value.RetlName );
				...
				//}
					...
					//console.log(top5RetlName);
					var top5RetlName = retlNames.filter(function(itm, i, a) {
						return i == retlNames.indexOf(itm);
					});

					var len = (top5RetlName.length > 5 ? 5 : top5RetlName.length);
				
					for(let i = 0;i<len;i++)
					{		 
						 $('#searched').append('<li><a>' + top5RetlName[i] + '</a></li>');
					}
					//$.each( top5RetlName, function( i, name ) {
					//  $('#searched').append('<li><a>' + name + '</a></li>');
					//});
					
						//register click event
					    $('#searched li').click(function(e) 
						{ 
						 $('#RetlName').val($(this).text());
						});
